    ...
    if (theoreticalAppointments < carePlan.estimatedAppointments) {
      ...
      nextAppointmentDate.setDate(
        carePlan.startDate.getDate() +
          (theoreticalAppointments + 1) * carePlan.daysBetweenAppointments,
      );
      nextAppointmentDate.setHours(0, 0, 0, 0);
      if (nextAppointmentDate.getTime() === today.getTime()) {
        console.log("La cita es hoy, programando la notificación para mañana");
        await sendEmail({
          ...
          message: `
            ...
            📅 **Fecha sugerida para la cita**: ${tomorrow.toLocaleDateString()}
